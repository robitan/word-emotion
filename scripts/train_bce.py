...
感情が近い単語ペアを学習し、学習済み埋め込みをQdrantに登録する。
"""
...
from src.training.bce import BCETrainer, EmotionPairDataset
...
def main():
    ...
    model_path = "checkpoints/bce_model.pth"
    ...
    # BCEコレクションを作成（既存の場合は再作成）
    db.create_collection("bce", recreate=True)
    ...
    # コレクション情報を表示
    info = db.get_collection_info("bce")
    print(f"\n[完了] BCE学習が完了しました")
